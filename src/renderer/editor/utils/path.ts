...
export function createPath(points: CurvePoint[], isClosed: boolean): Path {
    let hasBegin = false;
    ...
    function _isCornerRadius(idx: number) {
        ...
        return curvePoint.cornerRadius > 0;
    }
    ...
    function _getCornerInfo(idx: number): CornerCalcInfo {
        ...
        const minDist = Math.min(
            pre.cornerRadius > 0 ? lenAB / 2 : lenAB,
            next.cornerRadius > 0 ? lenBC / 2 : lenBC
        );
    ...
    function _connectTwo(fromIdx: number, toIdx: number) {
        ...
        if (_isCornerRadius(fromIdx)) {
            ...
            startHandle = fromCurvePoint.curveFrom ?? undefined;
        }
        ...
        if (_isCornerRadius(toIdx)) {
            ...
            endHandle = toCurvePoint.curveTo ?? undefined;
        }
...
export class CurvePoint {
    ...
    constructor(public point: Point = new Point()) {
    }
}
